...
track_grip = 1.0
visibility = 1.0
last_weather_check_time = 0.0 # New: To control weather update frequency
WEATHER_CHECK_INTERVAL_SEC = 300 # New: Check weather every 5 simulated minutes
...
last_engine_rpm = 0.0
# Store initial/normal values to reset to
...
INITIAL_HYBRID_BATTERY = 90.0 # Approx mid-range
...
def update_weather_system():
    ...
    global wind_speed, wind_direction, last_weather_check_time # Add new global
    
    # Update temperatures and wind continuously (smoother daily cycles)
    ...
    ambient_temperature = 20 + temp_variation + random.uniform(-0.5, 0.5) # Reduced jitter
    track_temperature = ambient_temperature + 8 + random.uniform(-1, 1) # Reduced jitter
    
    # Update wind (more dynamic)
    wind_speed = random.uniform(5, 25)
    wind_direction = random.randint(0, 359)

    # Only attempt a weather change at specific intervals to prevent rapid flickering
    if (total_simulated_time_sec - last_weather_check_time) < WEATHER_CHECK_INTERVAL_SEC:
        return # Not time for a weather check yet

    last_weather_check_time = total_simulated_time_sec # Update last check time
    
    weather_change_chance = 0.25 # Increased chance *when checking*, but checks are less frequent.
                                 # This makes it more likely to change when it *does* check.

    # Apply hourly modifiers to the weather change chance
    for hour_range, modifiers in HOURLY_WEATHER_MODIFIERS.items():
        if current_hour in hour_range:
            if "rain" in current_weather:
                weather_change_chance *= 0.3  # Rain tends to persist, less likely to change *from* rain
            if modifiers.get("rain_chance", 0) > 0 and "rain" not in current_weather:
                weather_change_chance *= (1 + modifiers["rain_chance"]) # Increase chance of rain if it's a rainy period
            if modifiers.get("fog_chance", 0) > 0 and "fog" not in current_weather:
                weather_change_chance *= (1 + modifiers["fog_chance"]) # Increase chance of fog
            break
    ...
    if random.random() < weather_change_chance:
        ...
        # Adjust probabilities slightly based on current weather to encourage persistence
        temp_weather_patterns = WEATHER_PATTERNS.copy()
        # If not clear, slightly make it harder to go to clear and easier to stay
        if current_weather != "clear":
            temp_weather_patterns["clear"]["probability"] *= 0.8 
            temp_weather_patterns[current_weather]["probability"] *= 1.2
        ...
        for weather_type, data in temp_weather_patterns.items():
            ...
            if weather_roll <= cumulative_prob:
                if weather_type != current_weather:
                    ...
                    # Apply new grip/visibility with slightly reduced jitter
                    track_grip = data["grip_factor"] + random.uniform(-0.03, 0.03)
                    visibility = data["visibility"] + random.uniform(-0.02, 0.02)
                    track_grip = max(0.4, min(1.0, track_grip)) # Clamp values
                    visibility = max(0.2, min(1.0, visibility)) # Clamp values
                break
...
def simulate_competitor_behavior():
    ...
    for car_id, competitor in HYPERCAR_COMPETITORS.items():
        ...
        # Weather impact
        weather_impact = (2.0 - track_grip) * 0.5 # Scale down impact slightly
        
        # Tire age impact
        tire_deg_impact = pos_data["tire_age"] * competitor["tire_degradation_rate"] * 0.1 # Scale down impact slightly
        
        # Fuel load impact
        fuel_ratio = pos_data["fuel_level"] / competitor["fuel_tank_liters"]
        fuel_impact = (1 - fuel_ratio) * 1.5 # Heavier = slower, inverse relationship (more fuel = less 1-ratio)
        
        # Driver skill variance (some drivers better in certain conditions)
        driver_variance = random.uniform(-0.5, 0.5) # Reduced driver variance jitter
        ...
        if "night_pace" in competitor.get("strengths", []) and is_night:
            driver_variance -= 0.5  # Better at night
        
        # Calculate final lap time for this hypothetical competitor's "current pace"
        final_lap_time = base_time + weather_impact + tire_deg_impact + fuel_impact + driver_variance
        
        # Update position relative to leader based on current pace difference over the interval
        if our_car_last_lap_time > 0 and final_lap_time > 0:
            # Calculate the time difference per full lap
            lap_time_difference = final_lap_time - our_car_last_lap_time
            # Apply this difference proportionally over the small 0.5s update interval
            gap_change_per_interval = lap_time_difference * (0.5 / our_car_last_lap_time)
            pos_data["gap_to_leader"] += gap_change_per_interval
            
            # Clamp gap to prevent extreme values
            pos_data["gap_to_leader"] = max(-300.0, min(300.0, pos_data["gap_to_leader"])) # E.g., +/- 5 minutes


        # Pit stop logic
        if (current_lap - pos_data["last_pit_lap"]) >= competitor["pit_frequency_laps"]:
            if random.random() < 0.15:  # 15% chance to pit this lap once eligible
        ...
        pos_data["tire_age"] += 1
        fuel_consumption = competitor["fuel_consumption_base"] * (1 + random.uniform(-0.05, 0.05)) # Reduced fuel jitter
        pos_data["fuel_level"] = max(0, pos_data["fuel_level"] - fuel_consumption)
        
        # Random reliability issues
        if random.random() < (1 - competitor["reliability_factor"]) / 5000: # Reduced chance of issue
            issue_types = ["engine_issue", "tire_problem", "aerodynamic_damage", "electrical_fault"]
            issue = random.choice(issue_types)
            pos_data["current_issues"].append(issue)
            pos_data["gap_to_leader"] += random.uniform(20, 50) # Increased impact to be noticeable
...
def generate_enhanced_telemetry():
    ...
    global last_speed_kmh, last_throttle_percent, last_brake_percent, last_engine_rpm # New globals for smoothing
    global oil_temp_C, water_temp_C # Ensure these are global if modified by anomaly system
    ...
    if sector_progress >= expected_sector_time:
        ...
        if current_sector == 1:  # Completed a lap (returned to S1)
            ...
            for tire in tire_wear: # tire_wear is a global variable
                ...
                tire_wear[tire] += wear_rate * random.uniform(0.9, 1.1) # Reduced jitter for wear
    ...
    # Calculate target values for smoothing
    ...
    if "straight" in sector_data["characteristics"]:
        target_speed_kmh = random.uniform(290, min(340, sector_data["max_speed_kmh"])) # Narrower range for target
    ...
    # Smoothing factor - higher for faster convergence, lower for smoother changes
    smoothing_factor = 0.15 # Adjust as needed (e.g., 0.1 to 0.3)
    
    # Apply smoothing and small random noise
    speed_kmh = last_speed_kmh + (target_speed_kmh - last_speed_kmh) * smoothing_factor + random.uniform(-2, 2)
    throttle_percent = last_throttle_percent + (target_throttle_percent - last_throttle_percent) * smoothing_factor + random.uniform(-0.5, 0.5)
    brake_percent = last_brake_percent + (target_brake_percent - last_brake_percent) * smoothing_factor + random.uniform(-0.5, 0.5)
    engine_rpm = last_engine_rpm + (target_engine_rpm - last_engine_rpm) * smoothing_factor + random.uniform(-50, 50)

    # Clamp values to realistic ranges
    speed_kmh = max(0, min(380, speed_kmh))
    throttle_percent = max(0, min(100, throttle_percent))
    brake_percent = max(0, min(100, brake_percent))
    engine_rpm = max(1000, min(11000, engine_rpm)) # Assuming some idle RPM
    ...
    # Apply weather and grip effects (after smoothing, before anomaly)
    if track_grip < 0.8:  # Wet conditions
        speed_kmh *= 0.85
    ...
    # NOTE: Direct manipulation by activate_anomaly takes precedence for critical states.
    # This section handles continuous modifiers if an anomaly is active but not explicitly forcing a value.
    ...
    for tire_pos in ["FL", "FR", "RL", "RR"]:
        # Only apply environmental/usage factors if no specific anomaly is directly setting temperature
        if not any(f"tire_puncture_{tire_pos.lower()}" in active_anomalies for anomaly_id in active_anomalies): # Check if any anomaly is active for this tire
            ...
            tire_temperatures[tire_pos] = current_base_temp * temp_modifier + random.uniform(-1, 1) # Reduced jitter for tire temps
        
        # Update tire pressures (temperature affects pressure)
        # Similar logic: only apply if not directly set by anomaly
        if not any(f"tire_puncture_{tire_pos.lower()}" in active_anomalies for anomaly_id in active_anomalies):
            temp_diff = tire_temperatures[tire_pos] - current_base_temp
            pressure_change = temp_diff * 0.01  # ~0.01 bar per degree
            base_pressure = INITIAL_TIRE_PRESSURES[tire_pos] # Use initial for reset reference
            tire_pressures[tire_pos] = base_pressure + pressure_change + random.uniform(-0.01, 0.01) # Reduced jitter for pressures
    ...
    for tire_pos in ["FL", "FR", "RL", "RR"]:
        travel = base_travel + random.uniform(-3, 3) # Reduced jitter for suspension
        ...
        if f"tire_puncture_{tire_pos.lower()}" in active_anomalies: # Check if specific tire puncture anomaly is active
            travel *= 1.5 # More significant travel for puncture
        suspension_travel[f"suspension_travel_{tire_pos}_mm"] = travel
    
    # Engine and drivetrain data
    # Only apply normal fluctuations if not actively overheating or fuel restricted
    if "engine_overheating" not in active_anomalies:
        oil_temp_C = INITIAL_OIL_TEMP + (engine_rpm - 7500) * 0.002 + random.uniform(-2, 2) # Reduced jitter
        water_temp_C = INITIAL_WATER_TEMP + (ambient_temperature - 20) * 0.5 + random.uniform(-1, 1) # Reduced jitter
    
    # Generate hybrid system data (for cars that have it)
    # Only apply normal fluctuations if not actively failed
    if "hybrid_system_failure" not in active_anomalies:
    ...
    else: # If hybrid system failure is active, force values
        hybrid_battery_percent = max(0, hybrid_battery_percent - 0.1) # Slowly drain or stay low
        hybrid_power_kw = 0 # No power output
    ...
    for car_id, competitor in HYPERCAR_COMPETITORS.items():
        ...
        competitor_data.append({
            ...
            "last_lap_time_sec": round(competitor["base_lap_time"] + random.uniform(-1, 1), 2), # Reduced jitter for competitor lap times
            ...
            "sector_time_1": round(competitor["base_lap_time"] * 0.35 + random.uniform(-0.5, 0.5), 2), # Reduced jitter
            "sector_time_2": round(competitor["base_lap_time"] * 0.30 + random.uniform(-0.5, 0.5), 2), # Reduced jitter
            "sector_time_3": round(competitor["base_lap_time"] * 0.35 + random.uniform(-0.5, 0.5), 2)  # Reduced jitter
        })
    ...
    if speed_kmh > sector_data["max_speed_kmh"] * 0.95 and random.random() < 0.05: # Chance of risk
        track_limits_warnings += 1 # Increment warnings
    ...
    telemetry_data = {
        ...
        "car": {
            ...
            "fuel_consumption_current_L_per_lap": round(OUR_CAR_CONFIG["fuel_consumption_base"] * (1 + random.uniform(-0.02, 0.02)), 2), # More dynamic consumption, less jitter
...
def update_anomalies():
    ...
    global active_anomalies, track_grip, current_weather, safety_car_active, visibility
    global tire_wear, tire_temperatures, tire_pressures, oil_temp_C, water_temp_C
    global last_speed_kmh, last_throttle_percent, last_brake_percent, last_engine_rpm
    ...
    for anomaly_type_id in expired_anomalies:
        ...
        # --- RESET AFFECTED PARAMETERS TO NORMAL RANGES ---
        if anomaly_config["type"] == "tire_puncture_front_left":
            tire_wear["FL"] = 0.0 # Reset wear
            tire_pressures["FL"] = INITIAL_TIRE_PRESSURES["FL"] # Reset pressure
            tire_temperatures["FL"] = INITIAL_TIRE_TEMPS["FL"] # Reset temperature
        elif anomaly_config["type"] == "hybrid_system_failure":
            # Reset hybrid system to normal operating state
            pass # These are generally self-regulating in generate_enhanced_telemetry if not overridden
        elif anomaly_config["type"] == "safety_car_period":
            safety_car_active = False
            # Speed will naturally increase due to target speed changes from sector characteristics
        elif anomaly_config["type"] == "sudden_weather_change":
            current_weather = "clear" # Reset to default clear
            track_grip = 1.0 # Reset to full grip
            visibility = 1.0 # Reset to full visibility
        elif anomaly_config["type"] == "engine_overheating":
            oil_temp_C = INITIAL_OIL_TEMP # Reset oil temp
            water_temp_C = INITIAL_WATER_TEMP # Reset water temp
            # Throttle will return to normal via smoothing as target changes back
        elif anomaly_config["type"] == "brake_balance_issue":
            tire_temperatures["FL"] = INITIAL_TIRE_TEMPS["FL"] # Reset specific tire temps
            tire_temperatures["FR"] = INITIAL_TIRE_TEMPS["FR"]
            tire_temperatures["RL"] = INITIAL_TIRE_TEMPS["RL"]
            tire_temperatures["RR"] = INITIAL_TIRE_TEMPS["RR"]
        # aerodynamic_damage doesn't force specific value changes, relies on lap time impact and speed reduction.
        elif anomaly_config["type"] == "fuel_flow_restriction":
            pass # Fuel consumption will return to normal in generate_enhanced_telemetry
            # Engine RPM will return to normal via smoothing
        # gearbox_sensor_fault relies on lap time impact and RPM smoothing
        elif anomaly_config["type"] == "night_visibility_issue":
            visibility = INITIAL_VISIBILITY # Reset visibility

        # Generic reset of last smoothed values to allow normal simulation to take over smoothly
        # This is more of a failsafe to ensure no lingering "bad" last_ values
        last_speed_kmh = random.uniform(200, 250) # Set to a normal mid-range for smooth recovery
        last_throttle_percent = random.uniform(60, 80)
        last_brake_percent = random.uniform(10, 30)
        last_engine_rpm = random.uniform(7000, 9000)


def activate_anomaly(anomaly_id: str, duration: int, message: str):
    """Activate a specific anomaly with effects"""
    global active_anomalies, current_weather, track_grip, safety_car_active, visibility
    global tire_wear, tire_temperatures, tire_pressures, oil_temp_C, water_temp_C
    global last_speed_kmh, last_throttle_percent, last_brake_percent, last_engine_rpm
    ...
    if anomaly_id == "0": # "reset_all_systems"
        # Reset all relevant global variables to their initial/normal states
        ...
        last_speed_kmh = 250.0
        last_throttle_percent = 70.0
        last_brake_percent = 15.0
        last_engine_rpm = 8000.0
    ...
    # --- APPLY IMMEDIATE AND CRITICAL EFFECTS TO CAR PARAMETERS ---
    if anomaly_config["type"] == "tire_puncture_front_left":
        tire_wear["FL"] = 0.9 # Critically high wear
        tire_pressures["FL"] = 0.5 # Critically low pressure
        tire_temperatures["FL"] = 120.0 # Critically high temperature
    elif anomaly_config["type"] == "hybrid_system_failure":
        last_speed_kmh *= 0.8 # Immediate speed drop
        last_engine_rpm *= 0.8 # Immediate RPM drop
        # Hybrid power and battery will be handled in generate_enhanced_telemetry loop now
    elif anomaly_config["type"] == "safety_car_period":
        safety_car_active = True
        last_speed_kmh = 80.0 # Force safety car speed
        last_throttle_percent = 20.0 # Low throttle
        last_brake_percent = 10.0 # Light braking
        last_engine_rpm = 4000.0 # Low RPM
    elif anomaly_config["type"] == "sudden_weather_change":
        current_weather = "heavy_rain"
        track_grip = 0.55 # Directly use the heavy rain grip
        visibility = 0.50 # Directly use the heavy rain visibility
        last_speed_kmh *= 0.7 # Significant speed reduction due to weather
        last_throttle_percent *= 0.6
        last_brake_percent *= 1.5 # More braking needed
    elif anomaly_config["type"] == "engine_overheating":
        oil_temp_C = 130.0 # Force critical oil temp
        water_temp_C = 105.0 # Force critical water temp
        last_throttle_percent = 40.0 # Force reduced throttle
        last_engine_rpm = 6000.0 # Force reduced RPM
        last_speed_kmh *= 0.85 # Speed reduction
    elif anomaly_config["type"] == "brake_balance_issue":
        tire_temperatures["FL"] += 10 # Increase front temperatures
        tire_temperatures["FR"] += 10
        tire_temperatures["RL"] -= 5 # Maybe slight drop in rear as balance shifts
        tire_temperatures["RR"] -= 5
        last_brake_percent = 90.0 # Force high brake input for limited effect
        last_speed_kmh *= 0.95 # Slight speed reduction
    elif anomaly_config["type"] == "aerodynamic_damage":
        last_speed_kmh *= 0.90 # Significant speed reduction
    elif anomaly_config["type"] == "fuel_flow_restriction":
        # Simulate by effectively increasing consumption or reducing available fuel
        our_car_fuel = max(0, our_car_fuel - 5.0) # Instant fuel drop
        # The fuel_consumption_current_L_per_lap will be calculated based on base, but impact will be seen on overall level
        last_engine_rpm *= 0.85 # Reduced engine power
        last_throttle_percent = 60.0 # Cannot get full throttle
    elif anomaly_config["type"] == "gearbox_sensor_fault":
        last_engine_rpm = random.uniform(4000, 10000) # Erratic RPMs
        last_speed_kmh = random.uniform(100, 200) # Erratic speed
    elif anomaly_config["type"] == "night_visibility_issue":
        visibility = 0.30 # Force low visibility
...
def enhanced_telemetry_generator(start_hour=14, total_race_hours=24):
    ...
    global oil_temp_C, water_temp_C, hybrid_battery_percent, hybrid_power_kw # Initialize these to initial values
    
    # Initialize simulation
    total_simulated_time_sec = start_hour * 3600
    lap_start_time_simulated = total_simulated_time_sec
    
    # Initialize temperatures and hybrid system
    oil_temp_C = INITIAL_OIL_TEMP
    water_temp_C = INITIAL_WATER_TEMP
    ...
    while total_simulated_time_sec < max_simulated_time_sec and not stop_simulation_event.is_set():
        ...
        try:
            ...
            try:
                ...
                # else: # Commented out to reduce console spam during normal operation
